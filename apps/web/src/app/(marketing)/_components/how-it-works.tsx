    ...
    <section id="how-it-works" className="px-6 py-20">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative grid gap-8 md:grid-cols-3">
          ...
          <div className="absolute left-0 right-0 top-8 hidden h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent md:block" />

          {steps.map((step) => (
            <div key={step.number} className="relative flex flex-col items-center text-center">
              <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full border-2 border-blue-100 bg-white shadow-sm">
                <span className="font-display text-xl font-bold text-blue-600">{step.number}</span>
              </div>
              <h3 className="font-display mb-3 text-lg font-semibold text-slate-900">
                {step.title}
              </h3>
              <p className="font-body text-sm leading-relaxed text-slate-600">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
