...
## Current status (last updated: 2026-05-17 Day 55)
...
- [2026-05-17] **Day 51 — pgvector audit**: confirmed `CREATE EXTENSION IF NOT EXISTS vector`, `embedding vector(1536)`, `embedding_model text`, and HNSW cosine index (`vector_cosine_ops`, m=16, ef_construction=64) were already in migration 0013; ERD §8.6 synced — fixed constraint from `UNIQUE(document_version_id, chunk_index)` to `UNIQUE(document_id, chunk_index)`, added `chunking_strategy`/`start_char_index`/`end_char_index` from migration 0016, corrected vector dimension to `VECTOR(1536)`; no code changes needed
...
- [2026-05-17] **Day 54-55 — RAG citations table, chat integration, tests, ADR**: migration 0017 adds `ai_message_sources` table (links assistant messages to retrieved chunks, `relevance_score NUMERIC(5,4)`, `citation_label`, UNIQUE(ai_message_id, document_chunk_id), RLS enabled); `aiMessageSources` Drizzle table + `AiMessageSource`/`NewAiMessageSource` types exported from `@ai-workspace-lab/db`; `POST /api/orgs/[orgSlug]/chat` extended with optional `documentId` body field — validates scope via `validateRagScope` (404 on miss), embeds query via `embedQuery` (non-fatal on failure), retrieves top-5 chunks at similarity ≥ 0.5 via `retrieveRelevantChunks`, injects system prompt via `buildContextBlock` when chunks found; in `onFinish` inserts `ai_message_sources` rows non-fatally; `ChatInterface` accepts `documentId`/`documentName` props, passes `documentId` in `useChat` body, shows "Document context" badge when set; 4 new unit tests (validateRagScope 404, no-chunks skips system prompt, chunks inject system prompt, sources insert in onFinish); integration test suite for `ai_message_sources` (insert, unique constraint, FK violations, cross-org isolation); `docs/adr/0013-rag-v1-with-pgvector.md`

### In progress
- Sprint 9: complete — embeddings, retrieval, RAG citations shipped

### Up next
- Day 56+: expose citations in streaming UI (data stream annotations); RLS policy for ai_message_sources; multi-document retrieval
