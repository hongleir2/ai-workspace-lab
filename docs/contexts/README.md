...
- [2026-05-17] **Day 38 — Document upload API endpoint** (`POST /api/orgs/[orgSlug]/documents`): creates document row + presigned R2 URL, returns 429 on quota exceeded (document_count limit), 402 on no subscription, 403 on feature disabled, 200 on success; 2-phase client flow (POST for metadata, PUT file to presigned URL); analytics event `document_upload_started`; 9 unit tests covering all error paths
