...
- **Day 40**: Document ownership tests (4 unit + 3 integration) covering FILE_TOO_LARGE/NOT_AUTHORIZED rejection, cross-org isolation, soft-delete exclusion; ADR 0010 (file storage + document ownership policy); dashboard entitlement-aware UI — shows plan-gated state with upgrade link when PostHog flag is on but plan doesn't include `document_uploads`; feature-gate sidebar nav — Documents and AI Chat nav items show lock icon + tooltip "Not available on your current plan" when PostHog flag is off; documents page returns 404 when flag is off; upload button gated by entitlement; `FLAGS` constant moved to `@ai-workspace-lab/analytics` (single source of truth, client-safe); tooltip component added
...
- **Day 45**: Tests + admin + ADR — 17 unit tests across `chunkText`/`extractText`/`processDocumentHandler`/`runWorkerOnce`; 5 route auth tests for `/api/internal/run-worker`; admin jobs page (`/admin/jobs`) showing all job statuses with error codes + attempt counts; `requirePlatformAdmin()` (email-allowlist via `ADMIN_EMAILS`) enforced at admin layout level; ADR 0011 (background job architecture); `docs/runbooks/queue-backlog.md`
- **Day 45 (additional)**: QStash two-mode automation + real-time UI — `triggerWorker()` fire-and-forget in document service (push trigger on upload via QStash → `/api/internal/run-worker`); QStash `Receiver` signature verification added to run-worker route (primary auth) + `WORKER_SECRET` bearer fallback; `AutoRefresh` client component (periodic `router.refresh()`); admin jobs page refreshes every 5s; document detail page refreshes every 3s while in non-terminal state; `@upstash/qstash` added to web app

Remaining:
- Embeddings: add vector generation in `processDocumentHandler` after chunking
- QStash cron safety net: configure schedule in QStash dashboard hitting `/api/internal/run-worker` every 2 min as fallback when push trigger is unavailable
